$(function() {
    ...
    function changeNav() {
        ...
        if ($('.header__nav').css('display') == 'none') {
            ...
            if ($('.header__nav-list').css('top') != '42px') {
                return false;
            };
        };
        var floorIndex = $('.active').index('.floor') - 1,
            navItem = $('.header__nav-item'),
            navListTop = Number($('.header__nav-list').css('top').slice(0, -2));
        
        if (navIndexBefore > navIndexAfter || navIndexAfter == undefined) {
            
            $('.header__nav-list').css({
                top: navListTop - 21
            });
        } else if (navIndexBefore < navIndexAfter) {

            $('.header__nav-list').css({
                top: navListTop + 21
            });
        }
        navItem.removeClass('header__nav-item_active');
        navItem.eq(floorIndex).addClass('header__nav-item_active');
    }
    ...
    if ($(window).width() > 1440) {
        ...
        $('.index-main').onepage_scroll({
            ...
            easing: 'ease',
            animationTime: 400, // Если менять время тут, то и в файле "_main-floor.sass" строка 31, у класса ".floor"(transition), для равномерной смены слайдов
            pagination: false,
            updateURL: false,
            loop: false,
