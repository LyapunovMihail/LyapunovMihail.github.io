...
// header
	var beforeElem,
		afterElem,
		blackLogoHeader = false; // При открытии панелей(топ просмотров, актупльное, поиск), для проверки наличия класса темного лого
	...
	$('.header__items-close').on('click', function() { // закрытие по клику на темное поле или кнопку
	...
	function clsAll() {
		...
		$('.header__item_menu').removeClass('header__item_cls-btn');
		$('.header').removeClass('header_menu-open');
		unFixedBody();
		if (blackLogoHeader) {
			$('.header').addClass('header_black-image');
			blackLogoHeader = false;
		}
	}
	...
	function firstClick(thisEl) {
		...
		$('.header__item_menu').addClass('header__item_cls-btn');
		$('.header__items-close').addClass('header__items-close_open');
		fixedBody();
		if( $('.header').hasClass('header_black-image') ) {
			$('.header').removeClass('header_black-image');
			blackLogoHeader = true;
		}
	}
    ...
    $('.header__item_menu').on('click', function() {
		...
		if ($('.header__item_menu').hasClass('header__item_cls-btn')) {
    ...
    function footerTransform(footerClass) {
		...
		footerClass ? fixedBody() : unFixedBody();
		footerClass ? $('.header').addClass('header_menu-open') : $('.header').removeClass('header_menu-open');
		$('.header__item_for-tooltip').removeClass('header__item_for-tooltip');
		$('.header__items-close').removeClass('header__items-close_open');
        ...
        if (footerClass && $(window).width() > 1440) {
			...
			$('.floor-footer').css('top', 100 * footerIndex + '%');
			unFixedBody();
			return;
		};
	};
	function footerReset() {
		...
		$('.floor-footer').removeClass('floor-footer_fixed');
		$('.header').removeClass('header_menu-open');
		footerClass = false;
		if (!$('body').hasClass('floor-body')) return;
...
// scroll-container
	function scrollContainer() {
		...
		$(window).scroll(function() {
			...
			if ($('.scroll-container__inner_sticky').position().top > 0) {
