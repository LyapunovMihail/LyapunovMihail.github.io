...
$(function() {
    var delaySlideMove = 800; // Если менять время тут, то и в файле "_main-floor.sass" строка 31, у класса ".floor"(transition), для равномерной смены слайдов
    ...
    function resizePlayer(iframes, ratio) {
        ...
        iframes.each(function(){
            ...
            current.width(playerWidth).height(height + 105).css({
                left: (width - playerWidth) / 2,
                top: 0 - 60
                });
            } else {
            playerHeight = Math.ceil(width / ratio);
            current.width(width).height(playerHeight + 105).css({
                left: 0,
                top: ((height - playerHeight) / 2) - 60
            });
            }
        });
    };
    
    resizePlayer($('.iframe-resize'), 16/9);

    if ($(window).width() > 1279 && $(window).height() > 870) {
    ...
    function movePath(el) {

        var thisIndex = el.index() + 1;

        $('html, body').animate( {scrollTop: $('.floor').eq(thisIndex).offset().top + 10}, 500 );
    }

    function swipeSlide(el) {

        if (el.prev().hasClass('header__nav-item_active')) {
            blockNavItem($('.header__nav-item'));
            $('.index-main').moveDown();
        } else if (el.next().hasClass('header__nav-item_active')) {
            blockNavItem($('.header__nav-item'));
            $('.index-main').moveUp();
        };
    }
    function blockNavItem(elems) {

        elems.css('pointer-events', 'none');

        setTimeout(function() {
            elems.css('pointer-events', '');
        }, delaySlideMove);
    };
});
$(function() {
...
// material-slider
	$('.material-slider').each(function(i, item) {
		...
		slider.slick({
			prevArrow: $(this).find('.material-slider__arrow--prev'),
			nextArrow: $(this).find('.material-slider__arrow--next'),
			fade: true,
			responsive: [
				{
					breakpoint: 450,
...
// category filter
	...
	$('.category__filter-setting').on('click', function() {

		if (!$(this).hasClass('category__filter-setting_active') && !$(this).hasClass('category__filter-setting_up')) {

			$('.category__filter-setting').removeClass('category__filter-setting_active');
			$('.category__filter-setting').removeClass('category__filter-setting_up');
			$(this).addClass('category__filter-setting_active');
			return;
		} else if ($(this).hasClass('category__filter-setting_active')) {
			$(this).addClass('category__filter-setting_up');
			$(this).removeClass('category__filter-setting_active');
			return;
		} else if ($(this).hasClass('category__filter-setting_up')) {
			$(this).addClass('category__filter-setting_active');
			$(this).removeClass('category__filter-setting_up');
			return;
		};
	});
});
...
// Автоувеличение input
$(function() {

    $('[data-input-autosize]').each(function() {
        $(this).on('input', function() {
            var inputSize = $(this),
                bufferSize = $('.longread-insert-buffer');

            bufferSize.css({
                font: inputSize.css('font'),
                'letter-spacing': inputSize.css('letter-spacing')
            });
            bufferSize.text(inputSize.val());
            inputSize.css('width', bufferSize.width());

            if( inputSize.next().next().length <= 0 ) {
                $('<div>', {
                    class: 'longread-insert-buffer'
                }).appendTo(inputSize.parent());
            };
        })
    })
});
