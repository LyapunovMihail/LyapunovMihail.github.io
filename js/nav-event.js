...
$(document).on('click', '.main__description-nav-item', function() {
    ...
    if ($('body').width() > 1069) {
        ...
        $(document).scrollTop(elem); // Перемещаюсь к якорю (фиксированное меню и навигация)
    } else {
...
$(window).on('scroll', function() {
    var $el = $('.active'), // Получаю активный элемент навигации
        ...
        if ( $prevEl != undefined && $('#' + $el.attr('data-id')).offset().top > $(window).scrollTop() + 150) {

            $('.main__description-nav-item').removeClass('active');
            $el.prev().addClass('active');
            scrllPointPrev($el)
        }
        if ( $nextEl != undefined && $('#' + $nextEl).offset().top < $(window).scrollTop() + 150) {
...
function stpBtnPast() {
    ...
    var $btnStop = $('[data-stop-past]').offset().top + 100 - 10,
        $btnStopMedia = $('[data-stop-past]').offset().top + 50 - 10,
        $height = $(window).outerHeight();
...
$(document).on('click', '[data-click]', function() {
    ...
    $('[data-animate]').addClass('subscr-animate');
});